	...
	private CdaMappingEngine getEngine() {
		if (engine == null) {
			try {
				...
				engine.loadPackage(getResourceAsStream("/cda-fhir-maps-300.tgz"));
				engine.loadPackage(getResourceAsStream("/ch-emed.tgz"));
...
// 	<effectiveTime value="20220330112426+0100"/>
	...
	void TestLab10() throws FHIRException, IOException {
		InputStream inputStream = getResourceAsStream("/mapped-Esempio CDA2_Referto Medicina di Laboratorio v10-230-fix.xml");
	...
	*/
	...
	void TestValidateCdaCh() throws FHIRException, IOException, EOperationOutcome {
		InputStream in = getResourceAsStream("/cda-ch.xml");
	...
	void TestValidateFhir() throws FHIRException, IOException, EOperationOutcome {
		InputStream in = getResourceAsStream("/pat.json");
		OperationOutcome outcome = getEngine().validate(in,
																		FhirFormat.JSON,
	...
	void TestRefertoDiLaboratoriFull() throws FHIRException, IOException {
		...
		Bundle bundleTransformed = getEngine().transformCdaToFhir(refertoDiLaboratori,
																					 "http://www.ey.com/italy/ig/cda-fhir-maps/StructureMap/RefertodilaboratorioFULLBODY");
		assertNotNull(bundleTransformed);
	}
	...
	private static InputStream getResourceAsStream(final String filename) {
		return CdaToFhirTransformTests.class.getResourceAsStream("/engine/transform" + filename);
	}
}
